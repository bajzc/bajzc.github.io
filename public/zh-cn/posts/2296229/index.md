...
一开始开了一个`ref_count[PHYSTOP/PGSIZE]`的大数组，然后想着全部初始化为0会方便一些，结果总是会被提前释放。
就改成了初始化为1，父进程的引用也算一次，逻辑上也更说得通。

最麻烦的是要通过`usertests`，很多组测试都会在`memmove`上出发`kerneltrap`。用gdb看了一下，参数`src`，和`dst`都会出现为0的情况。
说明复制COW页面的时候缺少了检查`pa`和`va`的步骤。

这里也发现了使用gdb的小技巧，可以先加载用户程序的符号，在测试函数那里断下。再加载内核符号，在`memmove`断下，就不用一步一步跳转了。
### To be continue...
